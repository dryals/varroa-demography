...
#SBATCH -o output/het.hwe.%a.%j.out  # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e output/het.hwe.%a.%j.err  # File to which STDERR will be written, %j inserts jobid
...
pop=$(cat ../pops/pop_list.txt | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
cat $SIM/$pop.filelist | while read bamfile
    do   	
        ...
        angsd -P 1 -i $bamfile -anc $REF -ref $REF -out het_est/$pop/$bam \
        	...
        	-sites ../../3_PCA/hwe_filter/varroa.edited.sites \
