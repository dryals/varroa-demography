...
#SBATCH -o outputs/likelihoods_%j.out  # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e outputs/likelihoods_%j.err  # File to which STDERR will be written, %j inserts jobid
...
mkdir -p first_angsd_out

echo 'making bamlist...'

cat samples_west.txt | while read sample
do
	echo ./bamfiles/$sample.bam >> bam.filelist
done
